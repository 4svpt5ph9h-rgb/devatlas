...
export default function AccountsPage() {
  ...
  useEffect(() => {
    ...
    authorizedFetch("/api/admin/users/").then(async response => {
      ...
      else setError(result.error || "Only the administrator can open this page.");
  ...
  async function createAccount(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await authorizedFetch("/api/admin/users/", {
        ...
        body: JSON.stringify({ name: fields.get("name"), email: fields.get("email"), password: fields.get("password") }),
      });
      const result = await response.json();
      if (!response.ok) {
  ...
  return (
    <main className="accounts-page">
      ...
      <section className="accounts-card" aria-labelledby="accounts-title">
        <div className="auth-header"><span className="auth-badge">Admin only</span><h1 id="accounts-title">Create account</h1><p>Add a person to DevAtlas.</p></div>
        {checking && <p role="status">Checking your access…</p>}
        {error && <p className="auth-error" role="alert">{error}</p>}
        {created && <div className="account-success" role="status"><strong>Account created</strong><p>{created.name} can now sign in with {created.email} and the initial password you chose.</p></div>}
        {allowed && !checking && <form className="auth-form" onSubmit={createAccount} aria-busy={busy}>
          ...
          <p className="form-note">New accounts are team members. They cannot create other accounts. This form does not send an email.</p>
